...
main:
  ...
  #####################################################################################
  #########Ejercicio2 pruebas de CORRECTO FUNCIONAMIENTO (Carlos y Adrian)#############
  add $t3, $t4, $t4 #r11=r12 + r12   == 16
  add $s2, $t4, $t3 #r18 =r12 + r11  == 24
  addi $t3, $t4, 1  #r11 = r12+1     == 9
  xor $s2, $s2, $s2 #comprobamos que el registro 18 queda con solo 0'
  and $t3, $t3, $t3 #el registro 11 no debe modificarse
  sub $s5, $t5, $t1 #r21= r13-r9   == 15
  or $t3, $t3, $t3	#r11 no se debe ver modificado
  nop
  nop
  nop
  nop
  #j final	#no se deberia ver cambios en r23 y en r9 y r10 tras esta instruccion
  slt $s7, $t1, $t2 #r23=1 si r9<r10   == 1
  slt $s7, $t2, $t1 #r23=1 si r10<r9   == 0
  slti $s7, $t2, 50  #r23=1 si r10<50	   == 1
  lui $t1, 8		#carga un 8 en la parte mas significativa (5 posicion al ser 32 bits)
  lui $t2, 8
  beq $t1, $t1, final 		#debe realizar el salto a #final
  #estas comprobaciones las veremos en las waves de ModelSim
  ###################################################################
  #####################################################################################
